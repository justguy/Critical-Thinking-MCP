...
> **Injected-defect gate measurement (Phase 1a-run / C, deterministic).** Because both Sonnet and Haiku produced ~0% natural defect density on the hard corpus, gate value is measured against *planted* defects instead. Running the settled gate over deterministic bundle mutations (`benchmark/run_live_gate_score.ts`, transcript `benchmark/reports/live_gate_score.json`): **block_recall 100%** (106/106 mutants), **false_block_rate 0%** (0/41 correct bases), **block_precision 100%**, `blocker_code_accuracy` 27.4% (a §7 labeling difference — a wrong final number is caught as `FINAL_ANSWER_ARTIFACT_DRIFT` and a fabrication as `SOURCE_SPAN_MISMATCH`, both correct catches under a different code than the C label). **Scope:** this is **output-level (§8) mutation only** — it proves the gate is a correct deterministic validator of structured defects with no false-blocks; it does **not** prove product value on live model workflows. `repair_success_rate`, task-level mutation, and the 4-arm forcing-vs-prompting ablation are deferred to the Phase 4 experiment.
