...
const outDir = resolve(
  repoRoot,
  'benchmark/duckexperiments/.ct_ab_clean_live_calibrated_2026-04-09',
);
...
function buildArgs(promptText: string, sessionMode: 'fresh' | 'continued'): string[] {
  const args = [
    '-p',
    '--verbose',
    '--output-format',
...
function runOne(
  ...
  const args = buildArgs(promptText, sessionMode);
  ...
  const calibration =
    arm === 'B'
      ? calibrateRun(round, promptId, finalResponseText, toolCalls)
...
function calibrationSummary(run: RunRecord): string[] {
  const result = run.calibrationResult;
  if (!result) {
...
function buildSection(run: RunRecord): string[] {
  ...
  if (run.calibrationEnvelope) {
    sections.push('**Calibration Envelope (Built Only From Real CT Inputs)**');
    sections.push('');
    sections.push(fenced(JSON.stringify(run.calibrationEnvelope, null, 2), 'json'));
    sections.push('');
  }

  if (run.calibrationResult) {
    sections.push('**Orchestrator Calibration Result**');
    sections.push('');
    sections.push(fenced(JSON.stringify(run.calibrationResult, null, 2), 'json'));
    sections.push('');
  }
...
function pairSummary(a: RunRecord, b: RunRecord): string[] {
  return [
    ...
    `- Arm B calibration decision: \`${b.calibrationResult?.policy_decision ?? 'not_run'}\``,
    `- Arm B calibration profile: \`${b.calibrationResult?.calibration?.profile_id ?? 'none'}\``,
    '',
    '**A Final Response**',
    '',
