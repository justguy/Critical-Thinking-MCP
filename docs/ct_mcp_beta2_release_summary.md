...
CT-MCP Beta 2 is validated as a deterministic control plane across two providers. It is not a claim that semantic truth is fully automated. It is a claim that the runtime now enforces boundaries, repairs what is repairable in one bounded turn, and halts when a model cannot be deterministically recovered.
...
The important result is not that every answer was released. The important result is that the same control plane produced the right terminal behavior for different model defaults:
