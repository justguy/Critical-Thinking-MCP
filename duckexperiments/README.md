...
This is intentionally separate from `benchmark/`.
...
- write artifacts under `duckexperiments/results/`
...
```bash
npm run duck:run -- --phase pilot
```

Useful variants:

```bash
npm run duck:run -- --phase pilot --profiles claude_low
npm run duck:run -- --phase full --profiles codex_low,codex_thinking
npm run duck:run -- --prompts C01,Q06 --profiles claude_low --max-cells 5
npm run duck:run -- --phase pilot --dry-run
```

If Gemini returns a valid answer in `stdout` but fails to write the artifact file, recover it with:

```bash
npm run duck:recover-gemini
```

Useful variants:

```bash
npm run duck:recover-gemini -- --dry-run
npm run duck:recover-gemini -- --profiles gemini_low
npm run duck:recover-gemini -- --profiles gemini_low,gemini_thinking --prompts C01,Q06
```

Notes:
- edit `duckexperiments/runner.config.json` once if you want to change model names, effort tiers, or the fixed `toolReviewHostProfile`
