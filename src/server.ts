...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  // Fresh engine per tool call — no shared state between invocations
  ...
  try {
    ...
    switch (name) {
      case 'validate_reasoning_chain':
        result = handleValidateReasoningChain(args, engine);
        break;
      case 'check_numeric_claims':
        result = handleCheckNumericClaims(args, engine);
        break;
      case 'detect_drift':
        result = handleDetectDrift(args, engine);
        break;
      case 'evaluate_tradeoffs':
        result = handleEvaluateTradeoffs(args, engine);
        break;
      case 'check_plan_validity':
        result = handleCheckPlanValidity(args, engine);
        break;
      case 'score_response_quality':
        result = handleScoreResponseQuality(args, engine);
        break;
      case 'validate_confidence':
        result = handleValidateConfidence(args, engine);
        break;
      case 'verify_arithmetic':
        result = handleVerifyArithmetic(args, engine);
        break;
      case 'detect_concurrency_patterns':
        result = handleDetectConcurrencyPatterns(args, engine);
        break;
      default:
        throw new McpError(
    ...
    const typedResult = result as { status?: string; enforcement?: { blocking_issues?: unknown[]; corrective_prompt?: string } };
    ...
    // Detect input validation errors by common patterns
